...
def sort_intersection_points(intersections):
    ...
    y_sorted = intersection_array[np.argsort(intersection_array[:, 1])]  # Sort by y-coordinate
    top_two = y_sorted[:2]  # Top two points
    bottom_two = y_sorted[2:]  # Bottom two points
    top_two_sorted = top_two[np.argsort(top_two[:, 0])]  # Sort top two by x-coordinate
    bottom_two_sorted = bottom_two[np.argsort(bottom_two[:, 0])]  # Sort bottom two by x-coordinate
    return np.concatenate((top_two_sorted, bottom_two_sorted))
