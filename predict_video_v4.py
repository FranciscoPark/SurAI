...
n_classes = 256
save_weights_path = 'WeightsTracknet/model.3'
...
# players tracker
dtype = get_dtype()
detection_model = DetectionModel(dtype=dtype) # detection_model instance 만들기
...
while True:
  ...
  if ret:
    if frame_i == 1:
        ...
        #### v3: added
        court_type2 = 2
        white_ref2 = court_detector.court_reference.get_court_mask(court_type2)
        white_mask2 = cv2.warpPerspective(white_ref2, court_detector.court_warp_matrix[-1], frame.shape[1::-1])
        image_court2 = frame.copy()
        image_court2[white_mask2 == 0, :] = (0, 0, 0)
    ...
    detection_model.detect_player_1(frame, image_court) # v3: parameter changed
    detection_model.detect_player_1_2(frame, image_court) # v3: parameter changed
    detection_model.detect_player_2(frame, image_court2) # v3: parameter changed
    detection_model.detect_player_2_2(frame, image_court2) # v3: parameter changed
...
player1_2_boxes = detection_model.player_1_2_boxes
player2_boxes = detection_model.player_2_boxes
player2_2_boxes = detection_model.player_2_2_boxes
...
for img in frames:
  ...
  img = mark_player_box(img, player1_2_boxes, frame_number)
  img = mark_player_box(img, player2_boxes, frame_number)
  img = mark_player_box(img, player2_2_boxes, frame_number)
...
while(True):
    ...
    print('player_1')
    output_img = mark_player_box(output_img, player1_2_boxes, currentFrame-1)
    print('player_1_2')
    output_img = mark_player_box(output_img, player2_boxes, currentFrame-1)
    print('player_2')
    output_img = mark_player_box(output_img, player2_2_boxes, currentFrame-1)
    print('player_2_2')
...
if bounce == 1:
  ...
  output_video = cv2.VideoWriter('/content/drive/MyDrive/tennis-tracking-main/output_bounce.mp4', fourcc, fps, (output_width, output_height))
  i = 0
  while True:
    ret, frame = video.read()
    if ret:
